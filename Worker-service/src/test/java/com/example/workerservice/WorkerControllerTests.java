...
import com.zaiika.workerservice.service.permission.CustomPermissionEvaluator;
...
class WorkerControllerTests {
    //Перед тестами отключить все 'hasPermission()' из-за отсутствия в тестах grpc
    @Autowired
    ...
    @MockBean
    private WebSecurityExpressionRoot securityRoot;

    @Test
    public void testGetAllWorkers() throws Exception {
        Mockito.when(securityRoot.hasPermission(null, "MANAGE_WORKER")).thenReturn(true);
        Mockito.when(userService.getUser()).thenReturn(new UserDto(1L, 99999));
    ...
    public void testCreateWorker() throws Exception {
        ...
        var body = writer.writeValueAsString(credentials);

        Mockito.when(userService.getUser()).thenReturn(new UserDto(99999, 99999));
    ...
    public void testDeleteWorker() throws Exception {
        Mockito.when(userService.getUser()).thenReturn(new UserDto(1L, 99999));
        mockMvc.perform(delete("/api/workers/99999").header("Authorization", "Bearer " + token))
                .andDo(print())
                .andExpect(status().isOk());
    }
    ...
    public void testGetWorkerById() throws Exception {
        Mockito.when(userService.getUser()).thenReturn(new UserDto(1L, 99999));
        mockMvc.perform(get("/api/workers/99999").header("Authorization", "Bearer " + token))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    public void testAddWorkerRole() throws Exception {
        Mockito.when(userService.getUser()).thenReturn(new UserDto(1L, 99999));
        mockMvc.perform(get("/api/workers/99999").param("roleName", "TEST PLACE ROLE")
                        .header("Authorization", "Bearer " + token))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(99999));
